...
public class XMLOutputTarget extends OutputHandler {
	...
	private String linebreak = PDFTextStreamConfig.getDefaultConfig().getLinebreakString();
	 ...
	 */
	public String getXMLAsString () throws IOException, TransformerException {
	 ...
	 * each PDF documents will be opened by {@link PDFTextStream}, and its content piped
	...
	public static void main (String[] args) throws Exception {
		...
		File src = new File("c:\\tmp\\test.pdf");
		if (!src.exists()) System.out.println("No such file");
		if (!src.canRead()) System.out.println("Cannot read file (check permissions): " + args[i]);
		//File destFile = new File(args[i] + ".xml");
		File destFile = new File("c:\\tmp\\test.xml");
		...
		System.out.println("Wrote text extracted from test.pdf" + " to " + destFile.getAbsolutePath() +
				" as UTF-8 encoded XML.");
		//}
	}
}